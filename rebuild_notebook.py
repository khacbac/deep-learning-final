...
NB = os.path.join(HERE, "notebooks", "gastrovision_classification.ipynb")
...
def main():
    ...
    if r.returncode != 0:
        ...
        sys.exit("build_notebook.py loi -> da phuc hoi ban sao luu")
    ...
    if len(base["cells"]) != len(fresh["cells"]):
        shutil.copy2(backup, NB)
        sys.exit("SO O DOI (%d -> %d): khong ghep duoc tu dong. Ban dung lai da bi phuc hoi;\n"
    ...
    for i, (a, b) in enumerate(zip(base["cells"], fresh["cells"])):
        if a["cell_type"] != b["cell_type"]:
            shutil.copy2(backup, NB)
            sys.exit("o %d doi loai (%s -> %s): khong ghep duoc. Da phuc hoi ban sao luu."
    ...
    if changed_code:
        ...
        sys.exit(
            ...
            "Ban dung lai da bi phuc hoi. Hay chay lai notebook tren Colab roi commit ban moi."
            % changed_code)
